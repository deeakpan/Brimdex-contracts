...
library Fixed192x64Math {
    ...
    /// @dev Returns bounds for value of 2**x given x (`lower`, `upper`).
    /// @param x exponent in fixed point
        ...
        returns (uint256 lower, uint256 upper)
    {
        ...
        shift -= 64;
        if (shift >= 0) {
            uint256 ushift = uint256(int256(shift));
            if (result >> uint256(256 - ushift) == 0) {
                lower = uint256(result) << ushift;
                zpow <<= uint256(int256(shift));
                if (lower + uint256(zpow) >= lower) upper = lower + uint256(zpow);
                else upper = type(uint256).max;
                return (lower, upper);
            } else return (type(uint256).max, type(uint256).max);
        }
        zpow = (zpow >> uint256(int256(-shift))) + 1;
        lower = uint256(result) >> uint256(int256(-shift));
    ...
    /// @dev Returns bounds for value of binaryLog(x) given x (`lower`, `upper`).
    /// @param x logarithm argument in fixed point
    ...
    /// @dev Returns base 2 logarithm floor of given x.
    /// @param x argument
    /// @return lo floor log2
    ...
    /// @return maxNum maximum element
    function max(int256[] memory nums)
        public
        pure
        returns (int256 maxNum)
    {
        require(nums.length > 0);
        maxNum = type(int256).min;
